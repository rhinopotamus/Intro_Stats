...
If you need to summarize a single categorical variable, a frequency table usually suffices. We'll use the `tally` command from the `mosaic` package to create all tables. Just like in the previous module, the tilde (~) stands for the word "by". Let's tally our data by race:
...
We can read this as "smoke *by* race", which makes sense: we are trying to predict smoking status *by* our knowledge of race.
